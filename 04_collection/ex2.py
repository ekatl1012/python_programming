...
a,b,c=zip(subjects, scores, str)
print(a,b,c)

for subject, socre in zip(subjects, scores):
...
# 길이가 5 이상인 이름만 뽑기
result=[len(name) for name in names if len(name)]
print(result)

# 중첩 for문도 가능
# print([for x in range(3) for y in range(3)])
...
result = [round(sum(scores)/len(scores) for scores in zip(std1, std2, std3))]
